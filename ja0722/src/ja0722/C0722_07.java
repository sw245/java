package ja0722;

import java.util.Scanner;

public class C0722_07 {

	public static void main(String[] args) {
		
		Scanner scan = new Scanner(System.in);
		
		/*
		// 조건 연산자: ?
		
		int x = -10;
		int absX = x>=0? x:-x;	// (condition)?(true_value):(false_value)
		System.out.println(absX);
		
		int n = scan.nextInt();
		String result = n%2==0?"짝수":"홀수";
		*/
		
		/*
		// 60점 합불 결과 출력
		
		System.out.println("숫자를 입력하세요.");
		int input = scan.nextInt();
		String result = input>=60? "합격" : "불합격";
		System.out.println("결과 : "+result);
		
		// 0보다 크다. 0이다. 0보다 작다.
		String result2 = (input>0)? "0보다 크다.": (input==0)? "0이다." : "0보다 작다.";	// 삼항식 중첩..
		System.out.println(result2);
		*/
		
		System.out.println("숫자 1을 입력하세요.");
		int n1 = scan.nextInt();
		
		System.out.println("숫자 2를 입력하세요.");
		int n2 = scan.nextInt();
		
		System.out.println("숫자 3을 입력하세요.");
		int n3 = scan.nextInt();
		
//		String result = (n1 > n2)? "숫자 1이 크다." : ((n1 == n2)? "숫자 1과 숫자 2는 같다." : "숫자 2가 크다.");
//		System.out.println(result);
		
		// 제일 큰 수 출력
		int result = (n1 > n2)? ((n1 > n3)? n1 : n3) : ((n2 > n3)? n2 : n3) ; 
		System.out.println("가장 큰 수 : "+result);
		System.out.printf("n1 : %d, n2 : %d, n3 : %d",n1,n2,n3);
	}

}
